...
| **D55** ~~(regla derogada por D61)~~ | La detección de bordes es Sobel + Hough sobre una miniatura de 320 px (~150 líneas). | Se escribió para evitar OpenCV.js por su peso. **Ese motivo ya no bloquea nada** (D61): la implementación propia se queda porque funciona y no tiene dependencias, no porque esté prohibido traerse algo mejor. Si OpenCV.js, Scanic o un modelo detectan mejor, entran. |
| **D56** ~~(regla derogada por D61)~~ | **Generador de PDF propio (~200 líneas)** en vez de `pdf-lib` o `jsPDF`. | El argumento del peso ya no vale (D61). El que sí se sostiene: el JPEG se incrusta **sin recodificar**, así que el archivo pesa lo mismo que las fotos y no pierde calidad. El día que haga falta texto seleccionable, OCR incrustado o firma digital, se trae `pdf-lib` sin discutir. |
...
| **D61** — ⚠️ **DEROGA el peso como criterio de rechazo** | **Dentro de la app no hay presupuesto de peso.** Ninguna función se descarta por lo que ocupe su librería. El tope de §3.1.1 se aplica **sólo a la landing pública**, y se cumple cargando en diferido, no renunciando a nada. | Decisión del dueño del producto, tomada expresamente. El argumento del peso se había convertido en un veto general y estaba impidiendo construir cosas mejores: OpenCV.js, un OCR de verdad, una librería de PDF completa. La distinción que lo resuelve: la portada decide si alguien se registra —ahí el peso es dinero— pero dentro de la app hay una persona que ya pagó, normalmente con wifi, descargando una vez algo que va a usar. Son dos problemas distintos y se estaban tratando como uno. **Regla operativa:** en la landing, todo lo pesado va tras `next/dynamic` y sólo se pide al usarlo; dentro de la app, se instala lo que haga falta. Lo único que se sigue vigilando es la memoria en dispositivos modestos —un búfer de 174 MB tumba el navegador aunque el código pese 2 KB (ver docs/evidencia-escaner.md)—, que es un problema de tiempo de ejecución y no de descarga. |
